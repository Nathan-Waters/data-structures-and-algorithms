...
const toLastNames = people => {
  return people.map(pep => pep.firstName + ' ' + pep.lastName);
};
...
const returnNames = (arr) => {
  return arr.reduce((starWars,starName) => {
    starWars.push(starName.name);
    return starWars;
  }, []);
};
...
const reversedString = (str) => {
  let array = str.split('');
  let reversal = array.reduce((newStr, back) => {
    return back + newStr;
  });
  return reversal;
};
